...
# ── FIX 5: Run the entire pipeline in ONE pass — no st.rerun() between steps ──
if run_btn:
    if not topic.strip():
        ...
        # Don't rerun here — fall through to the block below immediately
...
if st.session_state.running and not st.session_state.done:
    ...
    # Step 2: Reader (only if search succeeded)
    if "search" in results:
        with st.spinner("📄  Reader Agent is scraping top resources…"):
            try:
                ...
                results["reader"] = results["search"]   # fall back to search results
    ...
    st.rerun()   # single rerun only at the very end to refresh pipeline cards
